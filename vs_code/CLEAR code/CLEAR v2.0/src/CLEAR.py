...
import gc, sys, uasyncio, uio  # type: ignore
...
class Log():
    ...
    class Capture:
        ...
        class System:
            ...
            def memoryuse(self) -> None:
                memory:int=gc.mem_alloc()  # type: ignore
                mem=self.memory
                mem_tol=self.memory_tolrance
                if not (mem >= memory/1000 - mem_tol and mem <= memory/1000 + mem_tol ):
    ...
    def __init__(self):
        ...
        self.loop:str=""
        self.buffer=bytearray(2048)  # Buffer for log file to reduce write times.
    ...
    async def append_recording(self, entry:bytearray) -> None: # This is only ment for the recording.
        """
    ...
    async def append_log(self, entry: bytearray) -> None:
        """
        Appends to log file.
    ...
    async def brain_read(self, entry: bytearray|str) -> None:
        """
    ...
    def add(self, add_code: str, add_details: Any) -> None:
        ...
        code=self.codes.get(add_code, ":Code ERROR: Not Found. Code: %s"%add_code)
        brainscreen=self.brainscreen
        record=recording.record
        index=self.index
        adding=self.adding
        time_stamp=log_time.time

        if not adding:
            return
        
        entry= bytearray(b", %d [%d] %s %s \n" % (index, time_stamp(), code, add_details))
    ...
    async def auto_start_loop(self):
        ...
        except AttributeError: # used if the file is empty or doesnt exist to prevent crashes.
            addedfuntion=""
...
class Strip_Log():
    """
    An proformace log that can be used to log faster but with less details. Used for recording when the normal log would be too slow.
    """ 

    def Strip_CLEAR(self):
        """Used to delete all of the class Log, Recording, and Settings to fee memory."""
        global log, recording, settings
        del log, recording, settings

    @staticmethod
    def __call__(*args: Any, **kwds: Any) -> Any:
        Strip_Log().Strip_CLEAR()
        global logs
        logs=Strip_Log()
    
    def add(self, entry: str) -> None:
        """
        Main funtion for Strip_Log.

        Takes entry and adds it to the strip log file. 
        ...
        print(entry)
        brain.sdcard.appendfile("Strip_Log.txt", bytearray(entry + "\n", "utf-8"))
    
    def start(self, *args: Motor|Controller) -> None:
        """Starts the log. Enter motors or controllers you want to log."""

        while True:
            for item in args:
                if isinstance(item, Motor):
                    self.add("Motor: %s, Position: %d, Velocity: %d, Temperature: %d"%(item, item.position(DEGREES), item.velocity(PERCENT), item.temperature(PERCENT)))
                elif isinstance(item, Controller):
                    self.add("Controller: %s, Axis1: %d, Axis2: %d, Axis3: %d, Axis4: %d"%(item, item.axis1.position(), item.axis2.position(), item.axis3.position(), item.axis4.position()))
            wait(200, MSEC)
